...
def f(x, h = 0.6):
    return x * np.tan(x) - np.sqrt(h ** 2 - x ** 2)
...
for bound in (-0.6, 0.6):
    root, step = brent_solver(f, [0, bound])
    roots.append(root)
    steps.append(step)

# display the result
for i in range(2):
    message = 'root' + str(i) + ':' + str(roots[i]) + ' found after ' + str(steps[i]) + ' iterations'
    print(message)